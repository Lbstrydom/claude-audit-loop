...
| A known frontmatter key (`disable-model-invocation`, `allowed-tools`, …) is indented under a block scalar in any `.claude/skills/*/SKILL.md` | **repo** state | **BLOCKS** — `check-skill-frontmatter.mjs` in `skills:check` (source), `sync-to-repos` (refuses to deploy the bundle), `sync-isolation-verify` gate 9 / `doctor` probe `sync/skill-frontmatter-layout` (consumer-side, continuous) | YAML parses it as description TEXT: valid, silent, inert — a consumer's `/audit` declared `disable-model-invocation: true` while staying model-invocable (measured 2026-09-03). Gated on consumer-owned skills too, unlike gate 8: an inert declaration is never harmless and the fix is one dedent. |
