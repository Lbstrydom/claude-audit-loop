...
| `FINAL_REVIEW_MODEL` | No | `latest-flash` | The final reviewer's model sentinel or concrete ID. **Canonical name since 2026-09-07**; `GEMINI_REVIEW_MODEL` is a working, deprecated alias (canonical wins when both are set; the alias warns once when it contributes). Renamed because the role is provider-agnostic -- `FINAL_REVIEW_PROVIDER`, `selectProvider()`, an Opus fallback and a grok shadow arm all exist -- so naming its model after one vendor was legacy. The default moved `latest-pro` -> `latest-flash` the same day: [experiment-6](../research/experiment-6-adjudicator-swap-and-the-unreachable-rule.md). Read via `finalReviewModelSpec()` (config.mjs), which is the ONLY place the default lives -- a second copy in `gemini-review.mjs` silently reverted the tier switch at runtime. |
| `FINAL_REVIEW_TIMEOUT_MS` | No | `270000` | Final-review timeout -- sized for the CONSOLIDATED union-diff gate, not a per-cluster one (120s->180s 2026-07-31; 180s->270s 2026-08-10 after a 31-file union review timed out at 180s then ran 142s/130s). **COUPLED**: the watchdog floor is `2x timeout + 60000`, so 270s is the most the `FINAL_REVIEW_HARD_DEADLINE_MS` default admits -- raise both together. `GEMINI_REVIEW_TIMEOUT_MS` is a working, deprecated alias. Read via `finalReviewTimeoutMs()`. |
