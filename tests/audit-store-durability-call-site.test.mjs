...
const NOT_A_DURABLE_WRITE = {
  ...
  recordUpstreamIssueAnnotation: 'Source-side triage CLI write (cross-skill.mjs upstream annotate), never the orchestrator cloud block. Same ground as recordUpstreamIssue above: it ALREADY has its own durability mechanism — a write-ahead outbox on disk (.audit/upstream-annotation-outbox) drained on every subsequent upstream verb — which is a stronger guarantee than the spill queue, not a weaker one. Its replay is idempotent because the CALLER mints the row primary key and the insert arbitrates on it (ON CONFLICT (id) DO NOTHING); an annotation has no NATURAL unique key, since two identical corrections minutes apart are two legitimate rows, which is why the id is client-supplied rather than derived.',
