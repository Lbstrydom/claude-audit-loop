...
```bash
node scripts/plan-backend.mjs ${input:task}
```

Underlying script: `scripts/plan-backend.mjs` — same code path that Claude skills orchestrate, so output is consistent across both surfaces.
