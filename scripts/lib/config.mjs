...
/**
 * The two transports that can serve Claude in an Azure tenant. They are
 * DIFFERENT SERVICES, not two spellings of one — different host, different
 * credential, different auth header — so the endpoint and the key it is paired
 * with must be resolved together or not at all.
 *
 *   - `apim`    — an API Management front-end in front of Foundry. Base is the
 *                 AOAI/APIM endpoint + `/anthropic`; authenticates with the APIM
 *                 subscription key in an **`api-key`** header. A Bearer token is
 *                 rejected with "Access denied due to missing subscription key".
 *   - `foundry` — the direct AI Foundry inference endpoint. Base is
...
export function buildAzureConfig(env = process.env) {
  ...
  const claudeRoute = buildClaudeRoute(env, { openaiEndpoint, aiEndpoint, apiKey });
  return Object.freeze({
    active: true,
    openaiEndpoint,
    aiEndpoint,
    ...
    claudeApiShape,
    // The resolved Claude transport — endpoint, credential and auth header as
    // ONE unit. See buildClaudeRoute for why they cannot be resolved apart.
    claudeRoute,
    // Native-Anthropic base for the Claude path — the SDK appends `/v1/messages`,
    // yielding `…/anthropic/v1/messages`. Derived from `claudeRoute` so the base
    // and the credential can never disagree; kept as a named field because four
    // call sites and the test helpers read it.
    claudeBaseUrl: claudeRoute.baseUrl,
