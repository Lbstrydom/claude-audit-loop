...
> agent). Since VS Code 1.109 those skills surface directly as `/<name>` slash
> commands in Copilot Chat. [`AGENTS.md`](AGENTS.md) is picked up automatically
> as always-on project instructions. No configuration needed.
...
| GitHub Copilot Chat (VS Code, JetBrains) | `/skills` (Agent Skills auto-discovered from `.claude/skills/`) |
| Cursor / Windsurf Chat | `/skills` (Agent Skills open standard) |
